...
public class DirectoryContext : IDisposable
{
    ...
    public DirectoryContext(string dbFilePath, bool readOnly, string logDirectoryPath = null)
    {
        ...
        string jetInstanceName = string.Format(CultureInfo.InvariantCulture, JetInstanceNameFormat, Guid.NewGuid());
    ...
    {
        if (string.IsNullOrEmpty(path) || path.EndsWith(Path.DirectorySeparatorChar))
        {
