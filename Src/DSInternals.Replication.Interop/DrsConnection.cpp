...
namespace DSInternals::Replication::Interop
{
    ...
    midl_ptr<DRS_MSG_ADDSIDREQ_V1> DrsConnection::CreateAddSidHistoryRequest(
        String^ sourceDomain,
        String^ sourcePrincipal,
        String^ sourceDomainController,
        String^ destinationDomain,
        String^ destinationPrincipal,
        AddSidHistoryOptions flags,
        NetworkCredential^ sourceCredential)
    {
        auto request = make_midl_ptr<DRS_MSG_ADDSIDREQ_V1>();
        request->Flags = (UInt32)flags;
        request->SrcDomain = sourceDomain != nullptr ? RpcTypeConverter::ToNative(sourceDomain).release() : nullptr;
        request->SrcPrincipal = sourcePrincipal != nullptr ? RpcTypeConverter::ToNative(sourcePrincipal).release() : nullptr;
        request->SrcDomainController = sourceDomainController != nullptr ? RpcTypeConverter::ToNative(sourceDomainController).release() : nullptr;
        request->DstDomain = destinationDomain != nullptr ? RpcTypeConverter::ToNative(destinationDomain).release() : nullptr;
        request->DstPrincipal = destinationPrincipal != nullptr ? RpcTypeConverter::ToNative(destinationPrincipal).release() : nullptr;

        if (sourceCredential != nullptr)
        {
            String^ sourceCredentialUser = sourceCredential->UserName;
            String^ sourceCredentialDomain = sourceCredential->Domain;
            SecureString^ sourceCredentialPassword = sourceCredential->SecurePassword;

            if (sourceCredentialUser != nullptr && sourceCredentialUser->Length > 0)
            {
                request->SrcCredsUserLength = sourceCredentialUser->Length;
                request->SrcCredsUser = RpcTypeConverter::ToNative(sourceCredentialUser).release();
            }

            if (sourceCredentialDomain != nullptr && sourceCredentialDomain->Length > 0)
            {
                request->SrcCredsDomainLength = sourceCredentialDomain->Length;
                request->SrcCredsDomain = RpcTypeConverter::ToNative(sourceCredentialDomain).release();
            }

            if (sourceCredentialPassword != nullptr && sourceCredentialPassword->Length > 0)
            {
                request->SrcCredsPassword = (WCHAR*)Marshal::SecureStringToGlobalAllocUnicode(sourceCredentialPassword).ToPointer();
                request->SrcCredsPasswordLength = sourceCredentialPassword->Length;
            }
        }
    ...
    ReplicationResult^ DrsConnection::ReplicateAllObjects(ReplicationCookie^ cookie, array<ATTRTYP>^ partialAttributeSet, ULONG maxBytes, ULONG maxObjects)
    {
        ...
        auto objects = RpcTypeConverter::ReadObjects(reply->pObjects, reply->cNumObjects, reply->rgValues, reply->cNumValues, this->_schema);
	...
	void DrsConnection::AddSidHistory(
		String^ sourceDomain,
		String^ sourcePrincipal,
		String^ sourceDomainController,
		NetworkCredential^ sourceCredential,
		...
		AddSidHistoryOptions flags)
	{
        ...
        bool checkSecureChannel = flags.HasFlag(AddSidHistoryOptions::CheckSecureChannel);
        bool deleteSourceObject = flags.HasFlag(AddSidHistoryOptions::DeleteSourceObject);

        if (deleteSourceObject)
        {
            if (sourceDomain != nullptr && sourceDomain->Trim()->Length > 0)
            {
                throw gcnew ArgumentException("The source domain must not be specified when performing intraforest principal migration.", "sourceDomain");
            }

            if (destinationDomain != nullptr && destinationDomain->Trim()->Length > 0)
            {
                throw gcnew ArgumentException("The destination domain must not be specified when performing intraforest principal migration.", "destinationDomain");
            }

            if (sourcePrincipal == nullptr || sourcePrincipal->Trim()->Length == 0)
            {
                throw gcnew ArgumentException("The source principal cannot be null or empty.", "sourcePrincipal");
            }

            if (destinationPrincipal == nullptr || destinationPrincipal->Trim()->Length == 0)
            {
                throw gcnew ArgumentException("The destination principal cannot be null or empty.", "destinationPrincipal");
            }
        }
        else
        {
            if (!checkSecureChannel)
            {
                if (sourceDomain == nullptr || sourceDomain->Trim()->Length == 0)
                {
                    throw gcnew ArgumentException("The source domain cannot be null or empty.", "sourceDomain");
                }

                if (destinationDomain == nullptr || destinationDomain->Trim()->Length == 0)
                {
                    throw gcnew ArgumentException("The destination domain cannot be null or empty.", "destinationDomain");
                }
        ...
        auto request = this->CreateAddSidHistoryRequest(
            sourceDomain,
            sourcePrincipal,
        ...
        const DWORD inVersion = 1;
        auto reply = make_midl_ptr<DRS_MSG_ADDSIDREPLY_V1>();
        DWORD outVersion = 0;

        auto result = (Win32ErrorCode)IDL_DRSAddSidHistory_NoSEH(handle, inVersion, (DRS_MSG_ADDSIDREQ*)request.get(), &outVersion, (DRS_MSG_ADDSIDREPLY*)reply.get());

        // Check the primary error (RPC)
        Validator::AssertSuccess(result);

        // Check the secondary error (Active Directory)
        Validator::AssertSuccess((Win32ErrorCode)reply->dwWin32Error);
    }
