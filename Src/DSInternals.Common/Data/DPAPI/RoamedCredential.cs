...
public class RoamedCredential : DPAPIObject
{
    ...
    public RoamedCredential(byte[] blob, string accountName, SecurityIdentifier accountSid)
    {
        ...
        using (var stream = new MemoryStream(blob, false))
        {
            using (var reader = new BinaryReader(stream))
            {
                ...
                ArgumentOutOfRangeException.ThrowIfNotEqual(c1, '%', nameof(blob));
                ...
                ArgumentOutOfRangeException.ThrowIfNotEqual(c3, '\\', nameof(blob));
    ...
    public override string FilePath
    {
        get
        {
            ...
            sb.AppendFormat(CultureInfo.InvariantCulture, @"{0}\", this.AccountName);

            // Select the type-specific folder
            switch (this.Type)
            {
                case RoamedCredentialType.DPAPIMasterKey:
                    sb.AppendFormat(CultureInfo.InvariantCulture, @"Protect\{0}\", this.AccountSid);
                    break;
                ...
                case RoamedCredentialType.RSAPrivateKey:
                    sb.AppendFormat(CultureInfo.InvariantCulture, CapiRSAKeyDirectoryFormat, this.AccountSid);
                    break;
                case RoamedCredentialType.DSAPrivateKey:
                    sb.AppendFormat(CultureInfo.InvariantCulture, CapiDSAKeyDirectoryFormat, this.AccountSid);
                    break;
    ...
    public override string KiwiCommand
    {
        get
        {
            ...
            {
                case RoamedCredentialType.DPAPIMasterKey:
                    return this.Id != CurrentMasterKeyPointerId ? String.Format(CultureInfo.InvariantCulture, MasterKeyCommandFormat, this.FilePath, this.AccountSid) : null;
                case RoamedCredentialType.CNGCertificate:
                case RoamedCredentialType.CNGRequest:
                case RoamedCredentialType.CryptoApiCertificate:
                case RoamedCredentialType.CryptoApiRequest:
                    return String.Format(CultureInfo.InvariantCulture, CertificateCommandFormat, this.FilePath);
                case RoamedCredentialType.RSAPrivateKey:
                case RoamedCredentialType.DSAPrivateKey:
                    return String.Format(CultureInfo.InvariantCulture, CapiKeyCommandFormat, this.FilePath);
                case RoamedCredentialType.CNGPrivateKey:
                    return String.Format(CultureInfo.InvariantCulture, CNGKeyCommandFormat, this.FilePath);
                default:
