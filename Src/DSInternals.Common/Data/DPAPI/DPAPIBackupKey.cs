...
public class DPAPIBackupKey : DPAPIObject
{
    ...
    public DPAPIBackupKey(DirectoryObject dsObject, DirectorySecretDecryptor pek)
    {
        ...
        ArgumentNullException.ThrowIfNull(pek);
        ...
        get
        {
            switch (this.Type)
            {
                case DPAPIBackupKeyType.RSAKey:
                    // .pvk file
                    return String.Format(CultureInfo.InvariantCulture, RSAKeyFileNameFormat, this.KeyId);
                case DPAPIBackupKeyType.LegacyKey:
                    // .key file
                    return String.Format(CultureInfo.InvariantCulture, LegacyKeyFileNameFormat, this.KeyId);
                case DPAPIBackupKeyType.Unknown:
                    // Generate an additional random ID to prevent potential filename conflicts
                    int rnd = new Random().Next();
                    return String.Format(CultureInfo.InvariantCulture, UnknownKeyFileNameFormat, this.KeyId, rnd);
                default:
    ...
    public override string KiwiCommand
    {
        get
        {
            return this.Type == DPAPIBackupKeyType.RSAKey ? String.Format(CultureInfo.InvariantCulture, KiwiCommandFormat, this.FilePath) : null;
        }
    }
    ...
    public override void Save(string directoryPath)
    {
        ...
        switch (this.Type)
        {
            case DPAPIBackupKeyType.RSAKey:
                ...
                var pfxFile = String.Format(CultureInfo.InvariantCulture, RSAP12FileNameFormat, this.KeyId);
                fullFilePath = Path.Combine(directoryPath, pfxFile);
                File.WriteAllBytes(fullFilePath, pkcs12);

                // Create CER file
                var cerFile = String.Format(CultureInfo.InvariantCulture, RSACertFileNameFormat, this.KeyId);
    ...
    public static string PreferredRSAKeyName
    {
        get
        {
            return String.Format(CultureInfo.InvariantCulture, BackupKeyNameFormat, PreferredRSAKeyPointerName);
        }
    }
    ...
    public static string PreferredLegacyKeyName
    {
        get
        {
            return String.Format(CultureInfo.InvariantCulture, BackupKeyNameFormat, PreferredLegacyKeyPointerName);
        }
    }

    public static string GetKeyDN(Guid keyId, string domainDN)
    {
        return String.Format(CultureInfo.InvariantCulture, BackupKeyDNFormat, keyId, domainDN);
    }

    public static string GetKeyName(Guid keyId)
    {
        return String.Format(CultureInfo.InvariantCulture, BackupKeyNameFormat, keyId);
    }

    public static string GetPreferredRSAKeyPointerDN(string domainDN)
    {
        return String.Format(CultureInfo.InvariantCulture, BackupKeyDNFormat, PreferredRSAKeyPointerName, domainDN);
    }

    public static string GetPreferredLegacyKeyPointerDN(string domainDN)
    {
        return String.Format(CultureInfo.InvariantCulture, BackupKeyDNFormat, PreferredLegacyKeyPointerName, domainDN);
    }
