...
public class KdsRootKey
{
    ...
    private ConcurrentDictionary<int, byte[]> L0KeyCache = new ConcurrentDictionary<int, byte[]>();
    ...
    public KdsRootKey(DirectoryObject dsObject)
    {
        ...
        if (version.HasValue)
        {
            ArgumentOutOfRangeException.ThrowIfNotEqual(version.Value, ExpectedRootKeyVersion, nameof(version));
        }
    ...
    public static DateTime GetKeyStartTime(int l0KeyId, int l1KeyId, int l2KeyId)
    {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(l1KeyId, L1KeyModulus);

        ArgumentOutOfRangeException.ThrowIfNegative(l2KeyId);
        ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(l2KeyId, L2KeyModulus);
    ...
    public static Dictionary<int, string> ParseKdfParameters(byte[] blob)
    {
        ...
        using (Stream stream = new MemoryStream(blob))
        {
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                ArgumentOutOfRangeException.ThrowIfNotEqual(version, ExpectedKdfParametersVersion);
    ...
    public static (byte[] p, byte[] g) ParseSecretAgreementParameters(byte[] blob)
    {
        ...
        using (Stream stream = new MemoryStream(blob))
        {
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                int expectedLength = SecretAgreementParametersMinSize + 2 * keySize;
                ArgumentOutOfRangeException.ThrowIfNotEqual(length, expectedLength);
    ...
    {
        ArgumentNullException.ThrowIfNull(configurationNamingContext);

        return string.Format(CultureInfo.InvariantCulture, RootKeyDistinguishedNameFormat, rootKeyId, configurationNamingContext);
    }
