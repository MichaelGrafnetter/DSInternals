...
>
  ...
  <xsl:template match="asn:Sequence">#pragma warning disable SA1028 // ignore whitespace warnings for generated code
...
<xsl:choose><xsl:when test="not(@emitType) or @emitType='struct' or @emitType='both'">            <xsl:value-of select="@name" /> decoded = default;<xsl:if test="asn:AsnType[@defaultDerInit] | *[@defaultDerInit]/asn:AsnType">
            ...
            AsnReader collectionReader;</xsl:if><xsl:apply-templates mode="DefaultFieldVerify" /></xsl:when><xsl:otherwise>            Value<xsl:value-of select="@name" /> decoded = default;
            AsnReader reader;<xsl:apply-templates mode="ValueDefaultFieldVerify" /></xsl:otherwise></xsl:choose>
        }
#endif
    }
...
<xsl:apply-templates mode="Encode" />
        ...
        internal static void Decode(ref AsnReader reader,<xsl:if test="$hasRebind &gt; 0"> ReadOnlyMemory&lt;byte&gt; rebind,</xsl:if> out <xsl:value-of select="@name" /> decoded)
        {
        ...
        internal static void Decode(ref AsnReader reader, Asn1Tag expectedTag,<xsl:if test="$hasRebind &gt; 0"> ReadOnlyMemory&lt;byte&gt; rebind,</xsl:if> out <xsl:value-of select="@name" /> decoded)
        {
            try
            {
        ...
        private static void DecodeCore(ref AsnReader reader, Asn1Tag expectedTag,<xsl:if test="$hasRebind &gt; 0"> ReadOnlyMemory&lt;byte&gt; rebind,</xsl:if> out <xsl:value-of select="@name" /> decoded)
        {
            decoded = default;
...
<xsl:apply-templates mode="ValueEncode" />
        ...
        internal static void Decode(scoped ref AsnReader reader, out Value<xsl:value-of select="@name" /> decoded)
        {
            Decode(ref reader, Asn1Tag.Sequence, out decoded);
        }

        internal static void Decode(scoped ref AsnReader reader, Asn1Tag expectedTag, out Value<xsl:value-of select="@name" /> decoded)
        {
            try
            {
                DecodeCore(ref reader, expectedTag, out decoded);
        ...
        private static void DecodeCore(scoped ref AsnReader reader, Asn1Tag expectedTag, out Value<xsl:value-of select="@name" /> decoded)
        {
            decoded = default;
            AsnReader sequenceReader = reader.ReadSequence(expectedTag);<xsl:if test="*[@explicitTag]">
            AsnReader explicitReader;</xsl:if><xsl:if test="*[@defaultDerInit]">
            AsnReader defaultReader;</xsl:if><xsl:if test="$hasSpanTryRead &gt; 0">
            ReadOnlyMemory&lt;byte&gt; tmpSpan;</xsl:if>
...
<xsl:apply-templates mode="ValueCollectionEnumerable" />    }
    ...
    <xsl:template match="asn:Choice">#pragma warning disable SA1028 // ignore whitespace warnings for generated code
using System;<xsl:if test="(asn:SequenceOf | asn:SetOf) and (not(@emitType) or @emitType='struct' or @emitType='both')">
using System.Collections.Generic;</xsl:if>
using System.Formats.Asn1;
using System.Runtime.InteropServices;
...
<xsl:apply-templates mode="EncodeChoice" />
        ...
        internal static void Decode(ref AsnReader reader, <xsl:if test="not(@rebind='false')">ReadOnlyMemory&lt;byte&gt; rebind, </xsl:if>out <xsl:value-of select="@name" /> decoded)
        {
            try
            {
        ...
        private static void DecodeCore(ref AsnReader reader, <xsl:if test="not(@rebind='false')">ReadOnlyMemory&lt;byte&gt; rebind, </xsl:if>out <xsl:value-of select="@name" /> decoded)
        {
            decoded = default;
...
<xsl:apply-templates mode="ValueEncodeChoice" />
        ...
        private static void DecodeCore(scoped ref AsnReader reader, out Value<xsl:value-of select="@name" /> decoded)
        {
            decoded = default;
            Asn1Tag tag = reader.PeekTag();<xsl:if test="*[@explicitTag]">
            AsnReader explicitReader;</xsl:if><xsl:if test="$hasSpanTryRead &gt; 0">
            ReadOnlyMemory&lt;byte&gt; tmpSpan;</xsl:if>
...
<xsl:apply-templates select="*" mode="ValueCollectionEnumerable" />    }
  ...
  <xsl:template match="asn:SequenceOf[@namespace] | asn:SetOf[@namespace]">#pragma warning disable SA1028 // ignore whitespace warnings for generated code
using System;
...
namespace <xsl:value-of select="@namespace" />
{
    ...
    internal partial struct <xsl:value-of select="@name" />
    {
        internal <xsl:apply-templates select="*" mode="CollectionElementType"/>[] Values;

        internal readonly void Encode(AsnWriter writer)
        {
            Encode(writer, <xsl:apply-templates select="." mode="DefaultTag" />);
        }

        internal readonly void Encode(AsnWriter writer, Asn1Tag tag)
        {
            writer.Push<xsl:choose><xsl:when test="self::asn:SetOf">SetOf</xsl:when><xsl:otherwise>Sequence</xsl:otherwise></xsl:choose>(tag);
            foreach (<xsl:apply-templates select="*" mode="CollectionElementType"/> item in Values)
            {<xsl:apply-templates select="*" mode="EncodeSimpleValue"><xsl:with-param name="name" select="'item'"/><xsl:with-param name="writerName" select="'writer'"/><xsl:with-param name="indent" select="'    '"/></xsl:apply-templates>
            }
            writer.Pop<xsl:choose><xsl:when test="self::asn:SetOf">SetOf</xsl:when><xsl:otherwise>Sequence</xsl:otherwise></xsl:choose>(tag);
        }

        internal static <xsl:value-of select="@name" /> Decode(ReadOnlyMemory&lt;byte&gt; encoded, AsnEncodingRules ruleSet)
        {
            return Decode(<xsl:apply-templates select="." mode="DefaultTag" />, encoded, ruleSet);
        }
        ...
        internal static void Decode(ref AsnReader reader, ReadOnlyMemory&lt;byte&gt; rebind, out <xsl:value-of select="@name" /> decoded)
        {
            Decode(ref reader, <xsl:apply-templates select="." mode="DefaultTag" />, rebind, out decoded);
        }

        internal static void Decode(ref AsnReader reader, Asn1Tag expectedTag, ReadOnlyMemory&lt;byte&gt; rebind, out <xsl:value-of select="@name" /> decoded)
        {
            try
            {
        ...
        private static void DecodeCore(ref AsnReader reader, Asn1Tag expectedTag, ReadOnlyMemory&lt;byte&gt; rebind, out <xsl:value-of select="@name" /> decoded)
        {
            decoded = default;
            AsnReader collectionReader = reader.Read<xsl:choose><xsl:when test="self::asn:SetOf">SetOf</xsl:when><xsl:otherwise>Sequence</xsl:otherwise></xsl:choose>(expectedTag);
            var tmpList = new List&lt;<xsl:apply-templates select="*" mode="CollectionElementType"/>&gt;();
            <xsl:apply-templates select="*" mode="CollectionElementType"/> tmpItem;

            while (collectionReader.HasData)
            {<xsl:apply-templates select="*" mode="DecodeSimpleValue"><xsl:with-param name="name" select="'tmpItem'"/><xsl:with-param name="readerName" select="'collectionReader'"/><xsl:with-param name="indent" select="'    '"/></xsl:apply-templates>
                tmpList.Add(tmpItem);
            }
  ...
  <xsl:template match="*[@defaultDerInit]" mode="DefaultFieldDef">
        internal static readonly byte[] <xsl:call-template name="DefaultValueField"/> = { <xsl:value-of select="@defaultDerInit"/> };
</xsl:template>

  <xsl:template match="*[@defaultDerInit]" mode="DefaultFieldVerify">

            reader = new AsnReader(<xsl:call-template name="DefaultValueFieldUsage"/>, AsnEncodingRules.DER);<xsl:apply-templates select="." mode="DecodeSimpleValue"><xsl:with-param name="readerName" select="'reader'"/></xsl:apply-templates>
            reader.ThrowIfNotEmpty();</xsl:template>

  <xsl:template match="*[@defaultDerInit]" mode="ValueDefaultFieldVerify">

            reader = new AsnReader(<xsl:call-template name="DefaultValueFieldUsage"/>, AsnEncodingRules.DER);<xsl:apply-templates select="." mode="ValueDecodeSimpleValue"><xsl:with-param name="readerName" select="'reader'"/></xsl:apply-templates>
            reader.ThrowIfNotEmpty();</xsl:template>
...
<xsl:apply-templates select="." mode="EncodeValue"><xsl:with-param name="indent" select="'    '"/></xsl:apply-templates>
  ...
  <xsl:template match="asn:AnyValue" mode="EncodeSimpleValue" xml:space="default">
    ...
    <xsl:if test="1" xml:space="preserve">
            ...
            <xsl:value-of select="$indent"/>{
            <xsl:value-of select="$indent"/>    throw new CryptographicException("ASN1 corrupted data.", e);
  ...
  <xsl:template match="asn:GeneralizedTime" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:if test="@omitFractionalSeconds" xml:space="preserve">
            ...
            <xsl:value-of select="$indent"/>{
            <xsl:value-of select="$indent"/>    throw new CryptographicException("ASN1 corrupted data.");
            <xsl:value-of select="$indent"/>}
</xsl:if>
  </xsl:template>
  ...
  <xsl:template name="RebindMemory" xml:space="default">
    ...
    <xsl:param name="indent"/>
    <xsl:if test="1" xml:space="preserve">
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = tmpSpan;</xsl:if>
  </xsl:template>
  ...
  <xsl:template name="DefaultValueDecoder"><xsl:if test="@defaultDerInit">
            ...
            {
                defaultReader = new AsnReader(<xsl:call-template name="DefaultValueFieldUsage"/>, AsnEncodingRules.DER);<xsl:apply-templates select="." mode="DecodeSimpleValue"><xsl:with-param name="readerName" select="'defaultReader'"/><xsl:with-param name="indent" select="'    '"/></xsl:apply-templates>
            }</xsl:if></xsl:template>
  ...
  <xsl:template name="ValueDefaultValueDecoder"><xsl:if test="@defaultDerInit">
            ...
            {
                defaultReader = new AsnReader(<xsl:call-template name="DefaultValueFieldUsage"/>, AsnEncodingRules.DER);<xsl:apply-templates select="." mode="ValueDecodeSimpleValue"><xsl:with-param name="readerName" select="'defaultReader'"/><xsl:with-param name="indent" select="'    '"/></xsl:apply-templates>
            }</xsl:if></xsl:template>
        ...
        internal readonly ref struct <xsl:value-of select="@name"/>Enumerable
        {
            ...
            internal ref struct Enumerator
            {
                ...
                internal Enumerator(ReadOnlySpan&lt;byte&gt; encoded, AsnEncodingRules ruleSet)
                {
                    if (!encoded.IsEmpty)
                    {
                        AsnReader outerReader = new AsnReader(encoded, ruleSet);
