...
public static class WDigestHash
{
    ...
    [SuppressMessage("Performance", "CA1850: Prefer static HashData method over ComputeHash", Justification = "Compatibility with .NET Framework must be maintained.")]
    [SuppressMessage("Security", "CA5351: Do Not Use Broken Cryptographic Algorithms", Justification = "Active Directory still uses MD5 hashes.")]
    public static byte[][] ComputeHash(SecureString password, string userPrincipalName, string samAccountName, string netBiosDomainName, string dnsDomainName)
    {
        ...
        ArgumentException.ThrowIfNullOrWhiteSpace(samAccountName);
        ...
        string logonName = $"{netBiosDomainName}\\{samAccountName}";
        ...
        using (var md5 = MD5.Create())
        {
            ...
            result[1] = md5.ComputeHash(GetBytes(samAccountName.ToLowerInvariant(), netBiosDomainName.ToLowerInvariant(), password));

            // Hash3: MD5(UPPER(sAMAccountName), UPPER(NETBIOSDomainName), password)
            result[2] = md5.ComputeHash(GetBytes(samAccountName.ToUpperInvariant(), netBiosDomainName.ToUpperInvariant(), password));

            // Hash4: MD5(sAMAccountName, UPPER(NETBIOSDomainName), password)
            result[3] = md5.ComputeHash(GetBytes(samAccountName, netBiosDomainName.ToUpperInvariant(), password));

            // Hash5: MD5(sAMAccountName, LOWER(NETBIOSDomainName), password)
            result[4] = md5.ComputeHash(GetBytes(samAccountName, netBiosDomainName.ToLowerInvariant(), password));

            // Hash6: MD5(UPPER(sAMAccountName), LOWER(NETBIOSDomainName), password)
            result[5] = md5.ComputeHash(GetBytes(samAccountName.ToUpperInvariant(), netBiosDomainName.ToLowerInvariant(), password));

            // Hash7: MD5(LOWER(sAMAccountName), UPPER(NETBIOSDomainName), password)
            result[6] = md5.ComputeHash(GetBytes(samAccountName.ToLowerInvariant(), netBiosDomainName.ToUpperInvariant(), password));
            ...
            result[8] = md5.ComputeHash(GetBytes(samAccountName.ToLowerInvariant(), dnsDomainName.ToLowerInvariant(), password));

            // Hash10: MD5(UPPER(sAMAccountName), UPPER(DNSDomainName), password)
            result[9] = md5.ComputeHash(GetBytes(samAccountName.ToUpperInvariant(), dnsDomainName.ToUpperInvariant(), password));

            // Hash11: MD5(sAMAccountName, UPPER(DNSDomainName), password)
            result[10] = md5.ComputeHash(GetBytes(samAccountName, dnsDomainName.ToUpperInvariant(), password));

            // Hash12: MD5(sAMAccountName, LOWER(DNSDomainName), password)
            result[11] = md5.ComputeHash(GetBytes(samAccountName, dnsDomainName.ToLowerInvariant(), password));

            // Hash13: MD5(UPPER(sAMAccountName), LOWER(DNSDomainName), password)
            result[12] = md5.ComputeHash(GetBytes(samAccountName.ToUpperInvariant(), dnsDomainName.ToLowerInvariant(), password));

            // Hash14: MD5(LOWER(sAMAccountName), UPPER(DNSDomainName), password)
            result[13] = md5.ComputeHash(GetBytes(samAccountName.ToLowerInvariant(), dnsDomainName.ToUpperInvariant(), password));
            ...
            result[15] = md5.ComputeHash(GetBytes(userPrincipalName.ToLowerInvariant(), String.Empty, password));

            // Hash17: MD5(UPPER(userPrincipalName), password)
            result[16] = md5.ComputeHash(GetBytes(userPrincipalName.ToUpperInvariant(), String.Empty, password));
            ...
            result[18] = md5.ComputeHash(GetBytes(logonName.ToLowerInvariant(), password));

            // Hash20: MD5(UPPER(NETBIOSDomainName\sAMAccountName), password)
            result[19] = md5.ComputeHash(GetBytes(logonName.ToUpperInvariant(), password));
            ...
            result[21] = md5.ComputeHash(GetBytes(samAccountName.ToLowerInvariant(), MagicRealm, password));

            // Hash23: MD5(UPPER(sAMAccountName), "Digest", password)
            result[22] = md5.ComputeHash(GetBytes(samAccountName.ToUpperInvariant(), MagicRealm, password));
            ...
            result[24] = md5.ComputeHash(GetBytes(userPrincipalName.ToLowerInvariant(), MagicRealm, password));

            // Hash26: MD5(UPPER(userPrincipalName), "Digest", password)
            result[25] = md5.ComputeHash(GetBytes(userPrincipalName.ToUpperInvariant(), MagicRealm, password));
            ...
            result[27] = md5.ComputeHash(GetBytes(logonName.ToLowerInvariant(), MagicRealm, password));

            // Hash29: MD5(UPPER(NETBIOSDomainName\sAMAccountName), "Digest", password)
            result[28] = md5.ComputeHash(GetBytes(logonName.ToUpperInvariant(), MagicRealm, password));
        }
        return result;
    }
