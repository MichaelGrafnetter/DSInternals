...
public static class OrgIdHash
{
    ...
    {
        Validator.AssertLength(ntHash, NTHash.HashSize, nameof(ntHash));
        Validator.AssertLength(salt, SaltSize, nameof(salt));
    ...
    {
        return string.Format(CultureInfo.InvariantCulture, HashFormat, salt.ToHex(false), Iterations, hash.ToHex(false));
    }
}
