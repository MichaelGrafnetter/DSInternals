...
namespace DSInternals.Common.Kerberos
{
    ...
    public struct TrustAuthenticationInformation
    {
        ...
        public static (TrustAuthenticationInformation authInfo, int bytesRead) Parse(ReadOnlyMemory<byte> blob)
        {
            ...
            {
                throw new ArgumentOutOfRangeException($"Blob is too small to contain a TrustAuthenticationInformation structure. Minimum size is {StructHeaderSize} bytes.", nameof(blob));
            }
            ...
            {
                throw new ArgumentOutOfRangeException($"Blob is too small to contain the AuthenticationInformation. Expected length is {authInfoLength} bytes, but only {blob.Length - currentPosition} bytes are available.", nameof(blob));
            }
