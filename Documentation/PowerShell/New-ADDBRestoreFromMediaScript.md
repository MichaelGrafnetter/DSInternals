...
function Main {
    ...
    switch($script:Phase)
    {
        ...
        5 {
            ...
            [DSInternals.Common.Data.DSAccount] $dcAccount = Get-ADDBAccount -SamAccountName 'LON-DC1$' `
            ...
            Set-ADDBAccountPasswordHash -ObjectGuid '9bb4d6f4-060a-4585-9f18-625774e7c088' `
                                        -NTHash $dcAccount.NTHash `
                                        -SupplementalCredentials $dcAccount.SupplementalCredentials `
                                        -DatabasePath 'C:\Backup\Active Directory\ntds.dit' `
                                        -LogPath 'C:\Backup\Active Directory' `
                                        -BootKey $currentBootKey `
                                        -Verbose `
            ...
            robocopy.exe 'C:\Backup\Active Directory' $ntdsParams.'DSA Working Directory' *.dit *.edb *.chk *.jfm /MIR /NP /NDL /NJS *>> $script:LogFile
            ...
            robocopy.exe 'C:\Backup\Active Directory' $ntdsParams.'Database log files path' *.log *.jrs /MIR /NP /NDL /NJS *>> $script:LogFile
        ...
        6 {
            ...
            [string] $targetPath = Join-Path -Path "$env:SYSTEMROOT\SYSVOL" -ChildPath 'domain'
            ...
            if(Test-Path -Path $sysvolAclBackupPath -PathType Leaf) {
                ...
                [string] $aclRestorePath = Join-Path -Path "$env:SYSTEMROOT\SYSVOL" -ChildPath 'domain\Policies'
