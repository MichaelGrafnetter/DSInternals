...
## DESCRIPTION
This cmdlet retrieves the DNSSEC zone signing key (ZSK) and key signing key (KSK) descriptors hosted in Active Directory by querying the target domain controller through LDAP.
Only the key metadata is returned; use the Export-ADSIDnsServerSigningKey cmdlet to export the private key material.
